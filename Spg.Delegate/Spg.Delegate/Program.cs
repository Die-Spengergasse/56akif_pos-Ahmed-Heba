...
class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        LambdaTest.ForEach(converted, (message) =>
        {
            Console.Error.WriteLine(message);
        });

        Console.WriteLine("Beispiel 2: Filter");
        decimal[] freezed = LambdaTest.Filter(converted, ( val) =>
        {
            return val < 273.15M;
        });
        LambdaTest.ForEach(freezed, (message) =>
        {
            Console.Error.WriteLine(message);
        });
        ...
        result = LambdaTest.ArithmeticOperation(2, 0, Divide,  (message) =>
        {
            Console.Error.WriteLine(message);
        });
        Console.WriteLine(result);
